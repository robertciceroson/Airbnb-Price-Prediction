...
def load_all():
    ...
    drop_cols = ["id","name","host_id","host_profile_id","host_name","last_review","number_of_reviews_ltm","license"]
...
with col1:
    borough = st.selectbox("Borough", boroughs, index=boroughs.index("Manhattan") if "Manhattan" in boroughs else 0)
...
st.subheader("NYC Airbnb Price Map")
borough_filter = st.multiselect("Filter by Borough", options=boroughs, default=boroughs)
map_df = df[df["neighbourhood_group"].isin(borough_filter)].copy()
map_df = map_df[["neighbourhood_group","neighbourhood","room_type","price","latitude","longitude"]].dropna()
if len(map_df) > 5000:
    map_df = map_df.sample(5000, random_state=42)

fig = px.scatter_mapbox(
    map_df, lat="latitude", lon="longitude", color="price",
    color_continuous_scale="RdYlGn_r", range_color=[0, df["price"].quantile(0.9)],
    hover_data={"neighbourhood":True,"room_type":True,"price":True,"latitude":False,"longitude":False},
    zoom=10, center={"lat":40.7128,"lon":-74.0060}, height=500, mapbox_style="carto-positron"
)
...
price_table = (
    ...
    .rename(columns={"neighbourhood_group":"Borough","neighbourhood":"Neighbourhood","room_type":"Room Type","price":"Median Price ($)"})
